...
scenarios = {"scenario_a" : ["Testing-a2-a4.csv", "Training-a1-a3.csv"],
             "scenario_b" : ["Testing-a1.csv", "Training-a1-a2.csv"],
             "scenario_c" :  ["Testing-a1-a2-a3.csv", "Training-a1-a2.csv"]}

merged_files_ready = True

def main2():
    for test_train_ds in scenarios:
        dir_path = test_train_ds
        test_data_path = (scenarios[test_train_ds])[0]
        train_data_path = (scenarios[test_train_ds])[1]
        print ("scenario directory path = " + dir_path)
        print ("testing set = "+ test_data_path)
        print ("training set = "+ train_data_path)
        
        if (merged_files_ready == False):
             dp.merge_test_train(dir_path +"/"+"test/",dir_path +"/"+"train/",dir_path +"/"+"test_train_merge/")

        
         
        X_train, y_train = dp.get_processed_data(dir_path +"/"+train_data_path,dir_path +"/"+"test_train_merge/",classType='binary')
        X_test, y_test = dp.get_processed_data(dir_path +"/"+test_data_path,dir_path +"/"+"test_train_merge/",classType='binary')
        
        
        print("X_test Shape in the orig = ",np.shape(X_test))
        print("Y_test Shape in the orig = ",np.shape(y_test))
        print("X_train Shape in the orig = ",np.shape(X_train))
        print("Y_train Shape in the orig = ",np.shape(y_train))

        if (merged_files_ready == False):
                continue

        ## Import Test dataset
        #dataset = pd.read_csv(dir_path +"/"+test_data_path,header=None)
        #X_test = dataset.iloc[:, 0:-2].values
...
#
        ...
        #dataset = pd.read_csv(dir_path +"/"+ train_data_path,header=None)
...
#
        #
        ## just to experiment - add 20 columns 
        ##temp = np.zeros((15017,20))
